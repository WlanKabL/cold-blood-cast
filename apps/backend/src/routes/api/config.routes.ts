...
router.put("/sensor/:id", authMiddleware, (req: Request, res: any, next: NextFunction) => {
    try {
        const user = req.user!;
        const allowed = hasPermission(user, "manageSensors", req.params.id as string);
        ...
        if (!isValidSensorConfig(update)) {
            return next(badRequest("Invalid SensorConfig structure"));
        }

        if (update.id !== id) {
            return next(badRequest("Sensor ID mismatch"));
        }
...
router.post("/app", authMiddleware, (req: Request, res: any, next: NextFunction) => {
    try {
        ...
        if (!isValidAppConfig(update)) {
            return next(badRequest("Invalid AppConfig structure"));
        }
