<template>
    <div class="mx-auto max-w-4xl space-y-6 p-6">
        ...
        <div class="animate-fade-in-up flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="text-fg truncate text-2xl font-bold tracking-tight">
                    {{
                        visit?.isAppointment
                            ? $t("pages.vetVisits.detail.appointmentDetails")
                            : $t("pages.vetVisits.detail.visitDetails")
                    }}
                </h1>
            ...
            <div v-if="visit" class="flex items-center gap-2">
                ...
                <UiButton
                    variant="ghost"
                    icon="lucide:pencil"
                    @click="router.push(`/vet-visits?edit=${visit.id}`)"
                />
                <UiButton
                    variant="danger"
                    icon="lucide:trash-2"
        ...
        <template v-else-if="visit">
            <UiTabs v-model="activeTab" :tabs="visitTabs">
                <!-- Info Tab -->
                <div v-if="activeTab === 'info'" class="space-y-6">
                    ...
                    <div class="glass-card rounded-xl p-6">
                        <div class="mb-4 flex items-center justify-between">
                            <h2 class="text-fg font-semibold">
                                {{ $t("pages.vetVisits.detail.visitInfo") }}
                            </h2>
                            <span
                                v-if="visit.isAppointment"
                        ...
                        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <dt class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.fields.pet") }}
                                </dt>
                                <dd class="text-fg mt-1 text-sm">{{ visit.pet?.name ?? "—" }}</dd>
                            </div>
                            <div>
                                <dt class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.fields.veterinarian") }}
                                </dt>
                                <dd class="text-fg mt-1 text-sm">
                                    {{ visit.veterinarian?.name ?? $t("pages.vetVisits.noVet") }}
                                    <span
                                        v-if="visit.veterinarian?.clinicName"
                                        class="text-fg-faint"
                                    >
                                        · {{ visit.veterinarian.clinicName }}
                                    </span>
                                </dd>
                            </div>
                            <div>
                            ...
                            <div>
                                <dt class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.fields.visitType") }}
                                </dt>
                                <dd class="text-fg mt-1 text-sm">
                                    {{ $t(`pages.vetVisits.types.${visit.visitType}`) }}
                                </dd>
                            ...
                            <div v-if="visit.reason" class="sm:col-span-2">
                                <dt class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.fields.reason") }}
                                </dt>
                    ...
                    <!-- Cost & Weight -->
                    <div
                        v-if="!visit.isAppointment && (visit.costCents || visit.weightGrams)"
                        class="glass-card rounded-xl p-6"
                    >
                        <h2 class="text-fg mb-4 font-semibold">
                            {{ $t("pages.vetVisits.detail.costAndWeight") }}
                        </h2>
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div v-if="visit.costCents">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-fg-faint text-xs font-medium uppercase">
                                            {{ $t("pages.vetVisits.fields.cost") }}
                                        </p>
                            ...
                            <div v-if="visit.weightGrams">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-fg-faint text-xs font-medium uppercase">
                                            {{ $t("pages.vetVisits.fields.weight") }}
                                        </p>
                                        <p class="text-fg text-lg font-bold">
                                            {{ visit.weightGrams }} g
                                        </p>
                    ...
                    <div v-if="visit.nextAppointment" class="glass-card rounded-xl p-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.fields.nextAppointment") }}
                                </p>
                    ...
                    <div v-if="visit.notes" class="glass-card rounded-xl p-6">
                        <h2 class="text-fg mb-3 font-semibold">
                            {{ $t("pages.vetVisits.detail.notesSection") }}
                        </h2>
                        <p class="text-fg-muted text-sm whitespace-pre-wrap">{{ visit.notes }}</p>
                    </div>
                </div>
                ...
                <div v-if="activeTab === 'followUps'" class="space-y-6">
                    <div
                        v-if="visit.sourceVisit || visit.followUps?.length"
                        class="glass-card rounded-xl p-6"
                    >
                        <h2 class="text-fg mb-4 font-semibold">
                            {{ $t("pages.vetVisits.detail.followUpChain") }}
                        </h2>
                        <div class="space-y-3">
                            <!-- Source visit -->
                            <NuxtLink
                            ...
                            >
                                <Icon
                                    ...
                                    class="text-fg-faint h-4 w-4 rotate-180"
                                />
                                <div class="flex-1">
                                    <p class="text-fg text-sm font-medium">
                                        {{
                                            $t("pages.vetVisits.detail.sourceVisit", {
                                                date: new Date(
                                    ...
                                    <p
                                        v-if="visit.sourceVisit.reason"
                                        class="text-fg-faint text-xs"
                                    >
                                        {{ visit.sourceVisit.reason }}
                                    </p>
                                </div>
                                <span class="text-fg-faint text-xs">{{
                                    $t(`pages.vetVisits.types.${visit.sourceVisit.visitType}`)
                                }}</span>
                            </NuxtLink>
                            ...
                            <template v-if="visit.followUps?.length">
                                <p class="text-fg-faint text-xs font-medium uppercase">
                                    {{ $t("pages.vetVisits.detail.followUps") }}
                                </p>
                                <NuxtLink
                                ...
                                >
                                    ...
                                    <div class="flex-1">
                                        <p class="text-fg text-sm font-medium">
                                            ...
                                            <span v-if="fu.reason" class="text-fg-faint">
                                                — {{ fu.reason }}</span
                                            >
                                        </p>
                                    </div>
                                    ...
                                    <span v-else class="text-fg-faint text-xs">{{
                                        $t(`pages.vetVisits.types.${fu.visitType}`)
                                    }}</span>
                                </NuxtLink>
                            </template>
                        </div>
                    ...
                    <div v-else class="glass-card flex flex-col items-center rounded-xl py-8">
                        <Icon name="lucide:git-branch" class="text-fg-faint mb-2 h-8 w-8" />
                        <p class="text-fg-muted text-sm">
                            {{ $t("pages.vetVisits.detail.noNotes") }}
                        </p>
                    </div>
                </div>
                ...
                <div v-if="activeTab === 'documents'" class="space-y-6">
                    <div class="glass-card rounded-xl p-6">
                            ...
                            >
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-fg truncate text-sm font-medium">
                                        {{ doc.label || fileNameFromUrl(doc.upload.url) }}
                                    </p>
                        ...
                        <div v-else class="flex flex-col items-center py-8">
                            ...
                            <p class="text-fg-muted text-sm">
                                {{ $t("pages.vetVisits.detail.noDocuments") }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <UiModal
                :show="showUpload"
                :title="$t('pages.vetVisits.detail.uploadDocument')"
                @close="closeUpload"
            >
                <form class="space-y-4" @submit.prevent="handleUpload">
                    ...
                    >
                        ...
                        >
                            <Icon
                                ...
                                class="text-fg-faint mx-auto mb-2 h-10 w-10"
                            />
                            <p class="text-fg-muted text-sm">
                                {{ $t("pages.vetVisits.detail.dropzone") }}
                            </p>
                            <p class="text-fg-faint mt-1 text-xs">
                                {{ $t("pages.vetVisits.detail.dropzoneHint") }}
                            </p>
                        </div>
                        <div v-else class="flex items-center justify-center gap-3">
                            <Icon
                                :name="
                                    isImageFile(selectedFile) ? 'lucide:image' : 'lucide:file-text'
                                "
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        <UiButton variant="ghost" @click="closeUpload">{{
                            $t("common.cancel")
                        }}</UiButton>
                        <UiButton type="submit" :loading="uploading" :disabled="!selectedFile">
                            {{
                                uploading
                                    ? $t("pages.vetVisits.detail.uploading")
                                    : $t("pages.vetVisits.detail.uploadDocument")
                            }}
                        </UiButton>
                    </div>
            ...
            <UiModal
                :show="showEditLabel"
                :title="$t('pages.vetVisits.detail.editLabel')"
                @close="showEditLabel = false"
            >
                <form class="space-y-4" @submit.prevent="handleUpdateLabel">
                    ...
                    />
                    <div class="flex justify-end gap-2 pt-2">
                        <UiButton variant="ghost" @click="showEditLabel = false">{{
                            $t("common.cancel")
                        }}</UiButton>
                        <UiButton type="submit" :loading="updatingLabel">{{
                            $t("common.save")
                        }}</UiButton>
                    </div>
                </form>
...
useHead({
    title: () =>
        visit.value?.pet?.name
            ? `${visit.value.pet.name} — ${t("pages.vetVisits.detail.visitDetails")}`
            : t("pages.vetVisits.detail.visitDetails"),
});
...
const visitTabs = computed(() => [
    { key: "info", label: t("pages.vetVisits.detail.tabs.info"), icon: "lucide:clipboard-list" },
    {
        key: "followUps",
        label: t("pages.vetVisits.detail.tabs.followUps"),
        icon: "lucide:git-branch",
    },
    {
        key: "documents",
        label: t("pages.vetVisits.detail.tabs.documents"),
        icon: "lucide:file-text",
        badge: documents.value?.length ? String(documents.value.length) : undefined,
    },
]);
