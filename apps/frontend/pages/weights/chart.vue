    ...
    <div class="mx-auto max-w-5xl space-y-6 p-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-fg text-2xl font-bold tracking-tight">
                    {{ $t("pages.weights.chart.comparison") }}
                </h1>
        ...
        <div class="flex flex-wrap items-end gap-4">
            ...
            <div class="min-w-0 flex-1">
                <label class="text-fg-faint mb-1 block text-xs font-medium">{{
                    $t("pages.weights.chart.selectPets")
                }}</label>
                    ...
                    <button
                        ...
                        :class="
                            selectedPetIds.includes(p.id)
                                ? 'bg-primary-500/20 text-primary-400 ring-primary-500/30 ring-1'
            ...
            <div>
                <label class="text-fg-faint mb-1 block text-xs font-medium">{{
                    $t("pages.weights.chart.dateRange")
                }}</label>
                <select
                    v-model="dateRange"
        ...
        <!-- Growth Rate Cards -->
        <div
            v-if="growthRates?.length"
            class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3"
        >
            <div v-for="gr in growthRates" :key="gr.petId" class="glass-card rounded-xl p-4">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        >
                            {{
                                $t(
                                    `pages.weights.chart.trend${gr.trend.charAt(0).toUpperCase() + gr.trend.slice(1)}`,
                                )
                            }}
                        </span>
                    </div>
                ...
                <div v-if="gr.latestRecord" class="text-fg-muted mt-2 text-xs">
                    {{ gr.latestRecord.weightGrams }} g ·
                    {{ new Date(gr.latestRecord.date).toLocaleDateString() }}
                </div>
            </div>
        </div>
    </div>
...
const { data: chartSeries, isLoading: chartLoading } = useQuery({
    ...
    queryFn: () =>
        api.get<WeightChartSeries[]>(
            `/api/weights/chart?petIds=${petIdsParam.value}${chartFromParam.value}`,
        ),
...
const { data: growthRates } = useQuery({
    ...
    queryFn: () =>
        api.get<GrowthRateResult[]>(`/api/weights/growth-rate?petIds=${petIdsParam.value}`),
    enabled: () => selectedPetIds.value.length > 0,
});
</script>
