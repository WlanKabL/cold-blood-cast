            ...
            <div class="profile-hero-card animate-fade-in-up mb-8 p-6 sm:p-8">
                <div class="relative z-10 flex flex-col items-center gap-5 sm:flex-row sm:items-start">
                    ...
                    <div class="min-w-0 flex-1 text-center sm:pt-1 sm:text-left">
                        ...
                        <p
                            v-if="userData.tagline"
                            class="text-fg-muted mt-1 text-sm italic"
                        >
                            {{ userData.tagline }}
                        </p>
                        <div
                        ...
                        >
                            ...
                            <span v-if="userData.keeperSince" class="flex items-center gap-1">
                                ...
                                {{ $t("userProfile.keeperSinceLabel", { date: keeperSinceFormatted }) }}
                            </span>
                        </div>
                        <p
                            v-if="userData.bio"
            ...
            <section v-if="userData.pets.length" class="animate-fade-in-up mb-8 delay-200">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <NuxtLink
                        ...
                        :class="pet.petSlug ? 'hover:ring-1 hover:ring-white/20 cursor-pointer' : ''"
                    >
                        <img
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p v-if="pet.species" class="text-fg-muted text-xs">
                                {{ pet.species }}
                                <span v-if="pet.morph" class="text-fg-faint">· {{ pet.morph }}</span>
                            </p>
...
async function fetchLikeStatus() {
    try {
        const res = await fetch(`${apiBase}/api/public/community/user/${encodeURIComponent(slug)}/like`);
        if (res.ok) {
            const json = await res.json();
            if (json.success) {
...
async function fetchComments() {
    try {
        const res = await fetch(`${apiBase}/api/public/community/user/${encodeURIComponent(slug)}/comments`);
        if (res.ok) {
            const json = await res.json();
            if (json.success) {
...
async function handleDeleteOwnComment(commentId: string) {
    ...
    try {
        ...
        const res = await fetch(`${apiBase}/api/public/community/user/${encodeURIComponent(slug)}/comments/${commentId}`, {
            method: "DELETE",
            headers,
            credentials: "include",
        });
...
const themeClass = computed(() =>
    userData.value ? `theme-${userData.value.themePreset}` : "",
);
