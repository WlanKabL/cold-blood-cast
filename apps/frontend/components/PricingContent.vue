<template>
    <div class="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6">
        <!-- Success / Cancel Banners (auth only) -->
        <div
            v-if="!isPublic && route.query.subscription === 'success'"
        ...
        <div
            v-if="!isPublic && route.query.subscription === 'canceled'"
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div class="glass-card flex flex-col rounded-2xl p-6">
                ...
                <template v-if="isPublic">
                    <NuxtLink
                        to="/register"
                        class="bg-primary-500/10 text-primary-400 hover:bg-primary-500/20 inline-flex w-full items-center justify-center rounded-lg px-4 py-2.5 text-[13px] font-medium transition"
                    >
            ...
            >
                ...
                <template v-if="isPublic">
                    <NuxtLink
                        to="/register"
                        class="bg-primary-500 hover:bg-primary-600 inline-flex w-full items-center justify-center rounded-lg px-4 py-2.5 text-[13px] font-semibold text-white transition"
                    >
                        {{ $t("landing.nav.getStarted") }}
                    </NuxtLink>
                </template>
            ...
            <div class="glass-card flex flex-col rounded-2xl border-amber-500/20 p-6">
                ...
                <template v-else>
                    ...
                    <UiButton
                        ...
                        :disabled="!paymentsActive"
                        @click="
                            subscribe(billingCycle === 'monthly' ? 'pro_monthly' : 'pro_yearly')
                        "
                    >
                        {{ $t("pages.pricing.upgrade") }}
                    </UiButton>
        ...
        >
            ...
            <template v-if="isPublic">
                <NuxtLink
                    to="/register"
                    class="bg-primary-500 hover:bg-primary-600 inline-flex items-center justify-center rounded-lg px-6 py-2.5 text-[13px] font-semibold text-white transition"
                >
                    {{ $t("landing.nav.getStarted") }}
                </NuxtLink>
            </template>
        ...
        <!-- Manage Subscription (auth only, paid users) -->
        <div v-if="!isPublic && effectiveTier !== 'free' && paymentsActive" class="text-center">
...
const authStore = props.isPublic ? null : useAuthStore();

const effectiveTier = computed(() => {
    if (props.isPublic || !authStore) return "free";
