<template>
    <div class="container mx-auto px-4 py-8">
        <div v-if="!hideBackButton" class="mb-0">
            <button
                class="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 text-sm transition-colors"
                @click="goBack"
            >
                <svg
        ...
        <div class="p-6">
            <div class="text-center max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
                <h2 v-if="title" class="text-3xl sm:text-4xl font-bold text-fg mb-4">
                    {{ title }}
                </h2>
                <p v-if="subtitle" class="text-fg-muted">
                    {{ subtitle }}
                </p>
            </div>
            <slot />
